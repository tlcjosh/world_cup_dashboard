...
│       └── fifa_rankings.json# Live FIFA World Ranking (auto-regenerated by Actions)
│   └── sounds/
...
- **`fifaRank`**: a static field on every team in `TEAM_MASTER_DATA` (both `app.js` and `update_tracker.js`), used as a fallback default — see "Refreshing rankings" below for the live source.
...
- **Refreshing rankings (automated)**: `src/data/fifa_rankings.json` holds the live ranking, fetched once at startup by `fetchFifaRankings()` and consulted ahead of the static `TEAM_MASTER_DATA.fifaRank` fallback by `fifaRankOf()` in both `app.js` and `update_tracker.js`. `update_tracker.js`'s `syncFifaRankings()` runs on every cron tick: it looks in `blueprint_data/` for the most recently dated `fifa_rankings_YYYY-MM-DD.html` snapshot (filenames sort chronologically as strings), and if it's newer than the one already baked into `fifa_rankings.json` (tracked via a `source` field), re-parses it with `parseFifaRankingsHtml()` (matching FIFA's 3-letter team codes — more reliable than display names, since 7 of 48 teams have name mismatches, e.g. "Korea Republic" vs. "South Korea") and commits the regenerated file. **To refresh mid-tournament**: just drop a new dated HTML snapshot of FIFA's ranking page into `blueprint_data/` on `main` — the next Actions run (within 5 minutes) picks it up automatically, no code changes needed.
