...
All three hero slots rotate together through a pool built by `computeHeroStats()`: Played, Goals scored, Goals/match, Clean sheets, Highest-scoring match, Cards shown, Fouls committed, Saves, Corners won, Offsides, Penalties scored. "Played" and the always-available goals/clean-sheets entries are unconditional; the rest are only added to the pool once at least one match has that data (`cardsKnown`/`foulsKnown`/`savesKnown`/`cornersKnown`/`offsidesKnown`/`penaltiesKnown` counters) — early in the tournament, before any ESPN stats exist, the pool falls back to just the always-available entries.
...
- `_espnStats` — every key from ESPN's `competitor.statistics[]` array, copied through generically (not an allowlist) plus card counts derived from `details[]`. Includes `possessionPct`, `totalShots`, `shotsOnTarget`, `wonCorners`, `yellowCards`, `redCards`, `offsides`, `penaltyKickGoals`, and others not currently surfaced anywhere (`passPct`, `totalTackles`, `blockedShots`, `interceptions`, etc. — available if a future stat needs them).
...
- `homeYellowCards`, `awayYellowCards`, `homeRedCards`, `awayRedCards`, `homeFouls`, `awayFouls`, `homeSaves`, `awaySaves`, `homeCorners`, `awayCorners`, `homeOffsides`, `awayOffsides`, `homePenaltyGoals`, `awayPenaltyGoals` — raw per-match counts (not deduction points) promoted to top-level fields from `_espnStats`/the card-counting loop above. Live preview while ESPN tracks the match; permanent once `update_tracker.js`'s `syncMatchStats()` bakes them in after `FINISHED`. Feeds the Dashboard hero's rotating stat pool — see "Dashboard Hero Stats" below.
...
| `_espnStats` | `{home:{}, away:{}}` | Every stat from ESPN's `competitor.statistics[]`, copied through generically by name (`possessionPct`, `totalShots`, `shotsOnTarget`, `wonCorners`, `offsides`, `passPct`, `totalTackles`, `penaltyKickGoals`, etc. — whatever ESPN includes that match), plus `yellowCards`/`redCards` derived from `details[]` (not in the stats array). |
...
| `homeFouls`, `awayFouls`, `homeSaves`, `awaySaves`, `homeCorners`, `awayCorners`, `homeOffsides`, `awayOffsides`, `homePenaltyGoals`, `awayPenaltyGoals` | number | Per-match totals pulled straight through from ESPN's `statistics[]` array (`foulsCommitted`, `saves`, `wonCorners`, `offsides`, `penaltyKickGoals`). Same live-preview / permanent-backfill pattern as `homeFairPlay`. |
