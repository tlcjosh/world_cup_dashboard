...
- `_espnStats` — every key from ESPN's `competitor.statistics[]` array, copied through generically (not an allowlist) plus card counts derived from `details[]`. Includes `possessionPct`, `totalShots`, `shotsOnTarget`, `wonCorners`, `yellowCards`, `redCards`, `passPct`, and others not currently surfaced anywhere (`offsides`, `penaltyKickGoals`, `totalTackles`, `blockedShots`, `interceptions`, etc. — available if a future stat needs them).
...
- `homeYellowCards`, `awayYellowCards`, `homeRedCards`, `awayRedCards`, `homeFouls`, `awayFouls`, `homeSaves`, `awaySaves`, `homeCorners`, `awayCorners`, `homePassPct`, `awayPassPct` — raw per-match counts/percentages promoted to top-level fields from `_espnStats`/the card-counting loop above. Live preview while ESPN tracks the match; permanent once `update_tracker.js`'s `syncMatchStats()` bakes them in after `FINISHED`. Feeds the Dashboard hero's rotating stat pool — see "Dashboard Hero Stats" below.
...
| `_espnStats` | `{home:{}, away:{}}` | Every stat from ESPN's `competitor.statistics[]`, copied through generically by name (`possessionPct`, `totalShots`, `shotsOnTarget`, `wonCorners`, `passPct`, `offsides`, `totalTackles`, `penaltyKickGoals`, etc. — whatever ESPN includes that match), plus `yellowCards`/`redCards` derived from `details[]` (not in the stats array). |
...
| `homeFouls`, `awayFouls`, `homeSaves`, `awaySaves`, `homeCorners`, `awayCorners`, `homePassPct`, `awayPassPct` | number | Per-match totals/percentages pulled straight through from ESPN's `statistics[]` array (`foulsCommitted`, `saves`, `wonCorners`, `passPct`). Same live-preview / permanent-backfill pattern as `homeFairPlay`. |
