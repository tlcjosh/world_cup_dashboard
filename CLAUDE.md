...
- **Pitfall confirmed by direct observation**: `boxscore.teams[].statistics[]` `penaltyKickGoals`/`penaltyKickShots` do **NOT** update live during an in-progress shootout (stayed `"0"`/`"0"` after multiple made kicks on both sides) — only finalize once the match is fully `FINISHED`. Live preview must use `shootout[]`; `penaltyKickGoals` is safe only for permanent backend backfill (see below).
...
- **Backend (permanent)**: `update_tracker.js`'s `syncMatchStats()` bakes in `homeShootoutScore`/`awayShootoutScore` from the boxscore's `penaltyKickGoals` stat (via `BOXSCORE_STAT_FIELDS`) once a match is `FINISHED` — safe to use there since, unlike the live case, the match is already over and the stat has finalized.
...
| `homeShootoutScore`, `awayShootoutScore` | number\|undefined | Penalty shootout score (count of made kicks), not the AET score. Live preview set by `fetchESPNCommentary()` from `_shootout`; permanent value baked in from `boxscore.teams[]`'s `penaltyKickGoals` stat by `syncMatchStats()` once `FINISHED`. The tiebreaker `resolveTeam()`/`resolveBracketPlaceholders()` use to resolve `[W##]`/`[L##]` bracket placeholders when `homeScore === awayScore`. See "Penalty Shootouts" above. |
