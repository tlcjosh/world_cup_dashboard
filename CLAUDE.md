...
All three hero slots rotate together through a pool built by `computeHeroStats()`: Played, Goals scored, Goals/match, Clean sheets, Highest-scoring match, Cards shown, Fouls committed, Saves, Corners won, Pass accuracy. "Played" and the always-available goals/clean-sheets entries are unconditional; the rest are only added to the pool once at least one match has that data (`cardsKnown`/`foulsKnown`/`savesKnown`/`cornersKnown`/`passPctKnown` counters) — early in the tournament, before any ESPN stats exist, the pool falls back to just the always-available entries.
...
- `homeYellowCards`, `awayYellowCards`, `homeRedCards`, `awayRedCards`, `homeFouls`, `awayFouls`, `homeCorners`, `awayCorners` — raw per-match counts promoted to top-level fields from `_espnStats`/the card-counting loop above, every 10s scoreboard poll. Live preview while ESPN tracks the match; permanent once `update_tracker.js`'s `syncMatchStats()` bakes them in after `FINISHED`.
...
| `homeFouls`, `awayFouls`, `homeCorners`, `awayCorners` | number | Per-match totals pulled straight through from the scoreboard endpoint's `statistics[]` array (`foulsCommitted`, `wonCorners`). Same live-preview / permanent-backfill pattern as `homeFairPlay`. |
| `homeSaves`, `awaySaves`, `homePassPct`, `awayPassPct` | number\|undefined | Same per-match pattern, but pulled from the `/summary` endpoint's `boxscore.teams[]` (`saves`, `passPct`) instead — that endpoint's stats array is the only place these two fields exist. Live preview only refreshes while in-play (`fetchESPNCommentary()`); permanent once `syncMatchStats()` bakes it in via a dedicated boxscore fetch. `passPct` is a 0–1 fraction (e.g. `0.84`), not a 0–100 percentage. Left `undefined` (not `0`) when ESPN's boxscore has no stats for that match — see Known Issues #12, this is common for matches more than a few hours old. |
...
12. **ESPN's `/summary` boxscore stats go empty for matches more than a few hours old**: `boxscore.teams[].statistics[]` (the only source for `saves`/`passPct`, see `homeSaves`/`homePassPct` above) reliably returns ~28 populated stats for a live or just-finished match, but comes back as an empty array for matches further in the past — confirmed empirically across 24 already-FINISHED group-stage matches synced by `update_tracker.js` (kickoffs spanning a full week), where only the single most-recently-finished match got real `saves` values and every older one came back completely empty. `syncMatchStats()` originally treated "no boxscore stat found" the same as "stat is genuinely zero" and baked in a literal `0`/`0` for both teams — silently wrong for a match with several goals (a keeper can't make zero saves in a 7-1 loss). Fixed by `parseBoxscoreStat()` (distinct from `parseTeamStat()`, which is still correct for the scoreboard endpoint's always-present fields like fouls/corners) returning `undefined` instead of `0` when the stat is missing, and `syncMatchStats()` assigning that `undefined` through explicitly — which clears any already-baked false zero too, since `JSON.stringify` drops `undefined` keys on write. Net effect: the Dashboard hero's cumulative "Saves"/"Pass accuracy" pool entries (`computeHeroStats()`, gated by `typeof === 'number'`) now only ever sum real values instead of being diluted by fabricated zeros from old matches ESPN no longer serves boxscore data for. Since this looks like a hard cutoff in ESPN's own data retention rather than a matching bug on our end, these older matches will likely stay permanently excluded from the saves/pass-accuracy pool entries rather than ever backfilling — there is no other dataset in the repo with historical saves data to fall back on.
