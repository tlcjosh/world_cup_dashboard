...
| `homeSaves`, `awaySaves`, `homePassPct`, `awayPassPct` | number | Same per-match pattern, but pulled from the `/summary` endpoint's `boxscore.teams[]` (`saves`, `passPct`) instead — that endpoint's stats array is the only place these two fields exist. Live preview only refreshes while in-play (`fetchESPNCommentary()`); permanent once `syncMatchStats()` bakes it in via a dedicated boxscore fetch. `passPct` is a 0–1 fraction (e.g. `0.84`), not a 0–100 percentage. |
